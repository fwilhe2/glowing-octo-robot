# tar 1.35 predates libacl 2.3.2, which added acl_get_file_at, acl_set_file_at and
# acl_delete_def_file_at to <sys/acl.h> — three names tar had already been using for its
# own static wrappers around the non-at forms, generated by including gnulib's at-func.c.
# They are not the same functions: libacl's take an at_flags argument tar's do not, so a
# build against any current libacl stops with "conflicting types for 'acl_get_file_at'".
# tar has had no release since 1.35 (July 2023) and libacl 2.3.2 is from 2024, so there
# is no pair of versions that avoids this; every distribution carries a patch for it and
# this is ours. Renaming tar's three is the smaller half — they are static, they appear
# in no other file, and all 15 occurrences are in this one.
#
# Inline rather than a patch file because build.sh is the only thing in packages/tar/
# that is bind-mounted into the builder, the same reason packages/kernel/build.sh writes
# its config fragments as heredocs.
#
# The sed is idempotent — \b does not match inside tar_acl_get_file_at, so a second run
# changes nothing — which matters because root build.sh skips the extract when the tree
# is already unpacked, so a local rebuild re-runs this against patched source.
#
# Deliberately unasserted. If a future tar renames these itself the sed correctly does
# nothing, and if anything else about the file moves, the compiler says so as loudly as
